...
Task::status Disperse::run(void * arg, int steps){
	LOG_INFO(log,"start  disperse task ");
	...
	double distToBall = numeric_limits<double>::infinity();

	while( ( !this->stopTask && (steps)!=0 )  ){

		//odsuwa robota od przeszkod za pomoca metody pol potencjalowych
		//konstruuje pole ujemne
		//LOG_INFO(log,"this->stopTask "<<this->stopTask<<" distToObstacle "<<distToObstacle<<" start  disperse for robot "<<this->robot->getRobotName()<<" minimal distance from obstacle "<<this->minimalDistanceToObstacle );
		...
		if( ( currTime = Videoserver::getInstance().updateGameState( gameState ) ) > lastTime  )
		{
			...
			//oblicz odleglosc od pilki
			distToBall = gameState->getBallPos().distance( currRobotPose );
			...
			//LOG_INFO(log," gradient from lib dx="<<g.x<<" dy="<<g.y<<" gradient calculated dx="<<gradient.x<<" dy="<<gradient.y);
			...
			LOG_TRACE( log,"gradient "<< gradient <<"set speed "<<velocity<<" distTo nearest obs "<<distToObstacle<< " nearestObsPose"<<nearestObsPose << " dist to ball "<<ballPose.distance( currRobotPose ) );


			this->robot->setGlobalSpeed( velocity,0,currRobotPose.get<2>() );
		}

		if( ( distToObstacle >  this->minimalDistanceToObstacle ) ){
			break;
		}
	...
	if( distToObstacle > this->minimalDistanceToObstacle /*&&  distToBall < this->minimalDistanceToObstacle */){
		this->stop();
		LOG_INFO( log,"end  disperse for robot "<<this->robot->getRobotName()<<" with status Task::ok"<<" distToObstacle "<<distToObstacle );
		return Task::ok;

	}

	LOG_INFO( log,"end  disperse for robot "<<this->robot->getRobotName()<<" with status Task::not_completed" );
