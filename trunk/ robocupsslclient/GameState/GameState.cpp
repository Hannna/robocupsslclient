...
void GameState::updateRobotVel(std::string name,std::pair<Vector2D,double> vel){
	...
	//RotationMatrix rmY(this->robots[name].pos.get<2>());
	//this->robots[name].v=rmY.Inverse()*vel.first;
	this->robots[name].v=vel.first;
	this->robots[name].w=vel.second;
}
...
std::vector<Pose> GameState::getEnemyRobotsPos(std::string robotName){
	std::vector<Pose> result;
	std::map<std::string,Robot>::iterator ii=this->robots.begin();
	for(;ii!=this->robots.end();ii++){
		if(ii->first.compare(robotName))
